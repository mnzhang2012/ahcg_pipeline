...
##----------------
    ...
    ggsave(filename=paste("boxplot.exons",genes.name[k],"jpg",sep="."), width = 6.7, height = 6.7)
##-----------------------------------
...
##---------------------------------- 
    ...
    write.csv(stat,file=paste("cutoff",genes.name[k],"csv",sep="."))
...
##---------------
 ...
 write.table(table.exons,file="cutoff.final.csv",row.names = FALSE,col.names = FALSE,sep=",")
}
